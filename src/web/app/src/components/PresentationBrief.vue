...
export default {
  ...
  methods: {
    ...
    toggleFilesNameDisplay() {
      let filesDivs = document.getElementsByClassName("files");

      for (let i = 0; i < filesDivs.length; i++) {
        if (filesDivs[i].style.display === "none") {
          filesDivs[i].style.display = "block";
        } else {
          filesDivs[i].style.display = "none";
        }
      }
    }
  },
