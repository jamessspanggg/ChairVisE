...
export default {
  ...
  methods: {
    ...
    fileUploadHandler: function(file, fileList) {
      ...
      let fileFormat = this.$store.state.dataMapping.data.formatType;
      if (fileFormat == 1) {
        this.$store.commit("setUploadedFileType", "EasyChair");
      } else if (fileFormat == 2) {
        this.$store.commit("setUploadedFileType", "SoftConf");
      }
      Papa.parse(file.raw, {
